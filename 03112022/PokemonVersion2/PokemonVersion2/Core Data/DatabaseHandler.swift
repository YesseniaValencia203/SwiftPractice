...
class DatabaseHandler {
    ...
    // have to referene persistent container in app delegate to access persistent store
    private let viewContext: NSManagedObjectContext!
    static let shared = DatabaseHandler()
    init() {
        viewContext = (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext
    }
    // anything that conforms to NSManagedObject and pass it in and want to return it if it exists
    // make sure it is stored in the entity
